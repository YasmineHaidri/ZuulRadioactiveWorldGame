...
public class Game
{
    ...
    private void createRooms()
    {
        ...
        addItemsInRooms(wasteland);
        // END_WORLD
    }
    public void addItemsInRooms(Room[][] wasteland){
        Random random= new Random();
        Item[] allItems=new Item[8];
        allItems[0]= null;
        allItems[1]= new Item("Nuke Winter","+7",3);
        allItems[2]= new Item("Basic Bunny","+4",5);
        allItems[3]= new Item("Radiogenic","+3",6);
        allItems[4]= new Item("Exposeya","-2",7);
        allItems[5]= new Item("Karrotjuice","+4",9);
        allItems[6]= new Item("Corrosion","-4",10);
        allItems[7]= new Item("Dirty-Bomb","double the radiation",12);
        
        for(int row = 0; row<3; row++){
            for (int col = 0; col < 4; col++){
                wasteland[row][col].addItem(allItems[random.nextInt(8)]);            
            }
        }
